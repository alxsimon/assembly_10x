...
rule btk_prepare_workdir:
    ...
    shell:
        ...
        SC=$( grep '>' {output.fa} | wc -l )
        SP=$( grep -v '>' {output.fa} | wc -m )
        ...
        ln -s {input[2]} results/blobtoolkit/{wildcards.sample}_{wildcards.version}/GM_1.fastq.gz
        ln -s {input[3]} results/blobtoolkit/{wildcards.sample}_{wildcards.version}/GM_2.fastq.gz
        """
